...
def load_canonical_samples(jsonl_path: str | Path) -> tuple[dict[str, DataSample], DatasetInfo]:
    ...
    Returns samples indexed by ``sample.sample_id`` and the dataset-level
    ``DatasetInfo`` object.
    """
    ...
    if dataset_info is None:
        raise RuntimeError(f"Manifest {path} does not contain a dataset_info record.")
...
def get_image_assets(sample: Optional[DataSample]) -> list[ImageAsset]:
    """Return image-like assets from a sample, if present."""
    if sample is None:
        return []
    ...
    return [asset for asset in assets if isinstance(asset, ImageAsset)]
...
def extract_bbox_annotations(sample: Optional[DataSample]) -> list[InstanceAnnotation]:
    """Extract instance annotations with bounding boxes from all image assets."""
    if sample is None:
        return []

    anns: list[InstanceAnnotation] = []
...
def match_boxes(
    ...
    *,
    ...
    Returns:
      matches: dicts with gt_idx, pred_idx and iou
...
def evaluate_prediction_file(
    *,
    ...
    """Compute detection/localization metrics for one prediction JSONL."""
    ...
    for sid in tqdm(sample_ids, total=len(sample_ids), desc=f"mIoU {model_name}"):
        ...
        positive_match_ious.extend([float(m["iou"]) for m in matches])

    miou_gt_penalized = safe_div(total_iou_penalized, total_gt_for_miou)
    mean_iou_positive_matches = float(np.mean(positive_match_ious)) if positive_match_ious else 0.0
...
def plot_metric_by_threshold(rows: list[dict[str, Any]], *, metric: str, out_path: Path) -> None:
    models = sorted({r["model"] for r in rows})

    plt.figure(figsize=(10, 6))

    for model in tqdm(models, desc=f"Plotting {metric}", total=len(models)):
        model_rows = sorted([r for r in rows if r["model"] == model], key=lambda r: r["threshold"])
...
def plot_summary_bar(summary_rows: list[dict[str, Any]], *, metric: str, out_path: Path) -> None:
...
def resolve_image_path(sample: DataSample, image_root: Optional[str | Path]) -> Optional[Path]:
    ...
    resolved = asset.resolve_path(to_absolute_path(str(image_root)) if image_root is not None else None)
...
def draw_box(
    ...
    *,
    ...
    ax.text(
        ...
        color=text_color,
        fontsize=fontsize,
        bbox={"facecolor": color, "alpha": 0.7, "edgecolor": "none", "boxstyle": "Round, pad=0.2"},
    )
    ...
    *,
    ...
    for ax, model_name in zip(axes[1:], model_names):
        ...
        for pi, pred in enumerate(pred_anns):
            ...
            draw_box(ax, bbox_xyxy(pred), label=label, color=color, linestyle="-")
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.get("visualization", {}).get("enabled", False):
        ...
        requested_sample_ids = [str(s) for s in (vis_cfg.get("sample_ids", []) or [])]
        ...
        for sid in tqdm(sample_ids, desc="Processing visualizations", total=len(sample_ids)):
